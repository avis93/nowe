...
int HoughDetection(const Mat& src_gray, const Mat& src_display, int cannyThreshold, int accumulatorThreshold)
{
	...
	for (int i = 0; i < 9; i++)
	{
		line(display, Point(0, flag), Point(width, flag), cv::Scalar(0, 255, 255));
		flag += stepSize;
	}
	flag = 0;
	for (int i = 0; i < 9; i++)
	{
		line(display, Point(flag, 0), Point(flag, height), cv::Scalar(0, 255, 255));
		flag += stepSize1;
	}
...
int main() {
    ...
    do {
        ...
        while (!won(&board, ai, false) && !won(&board, human, false) && !drawGame(&board, false)) {
            ...
            player = gameMove(&board, player,res);
        }
